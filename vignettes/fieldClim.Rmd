...
By setting `slope`, `sky_view` and `exposition`, sw_in and sw_out will adittionally be topographically corrected (see `?rad_sw_in_topo`).
...
If lw_in is not provided, it will get estimated using the air temperature and air pressure (see `?rad_lw_in`). By setting `sky_view`, sw_in will be topographically corrected (see `?rad_sw_in_topo`).
...
As you can see it is quite the effort to get all the appropriate parameters. You even have to calculate some (p1, p2) for yourself if they are not available.
...
A convenience function to calculate all the different methods for the turbulent fluxes is given in 
