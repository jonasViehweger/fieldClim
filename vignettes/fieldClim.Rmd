...
There are also two additional toggles that can be set, called `reduced` and `units`. `reduced` will omit some (very few) columns which aren't used often. `units` will add longer column names with the correct units to all columns available with `reduced = TRUE`.
...
units <- as.data.frame(station_turbulent, reduced = T, units = T)
colnames(units = T)
